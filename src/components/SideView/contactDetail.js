import React, { Component } from 'react';
import Button from 'material-ui/Button';
import { connect } from 'react-redux';
import { Avatar } from 'antd';
import FormDialog from '../Modal/dialog';
import config from '../../config/config';
import VedioCall from '../ChatApp/vedioCall';

class ContactDetail extends Component {
  state = {
    open: false,
  };

  handleClickOpen = () => {
    this.setState({ open: true });
  };

  handleClose = () => {
    this.setState({ open: false });
  };

  render() {
    const avatarURL = this.props.setCurrentFriend.avatarURL !== ''
      ? `${config.BASE_URL}/images/avatars/${this.props.setCurrentFriend.avatarURL}`
      : `${config.BASE_URL}/images/avatar_placeholder.png`;

    // if (this.props.setCurrentFriend.avatarURL !== undefined) {
    const friendDetail = (
      <div>
        <div style={{ float: 'right', marginLeft: 10 }}>
          <Avatar avatar="avatarURL" size="large" />
        </div>

        <Button onClick={this.handleClickOpen} style={{ float: 'right', color: 'white' }}>
          <i className="material-icons">phone</i>
        </Button>
        <FormDialog
          compo="Comming soon..."
          fullScreen={false}
          handleClickOpen={this.handleClickOpen}
          handleClose={this.handleClose}
          open={this.state.open}
        />
        <Button id="cypress-vedio" onClick={this.handleClickOpen} style={{ float: 'right', color: 'white' }}>
          <i className="material-icons">videocam</i>
        </Button>
        <FormDialog
          compo={<VedioCall />}
          fullScreen
          handleClickOpen={this.handleClickOpen}
          handleClose={this.handleClose}
          open={this.state.open}
        />

        <div />
      </div>
    );
    // }
    return (
      <div className="contact-list">
        <div style={{ width: '100%' }}>
          <div style={{ float: 'left' }}>
            <img alt="logo" src={`${config.BASE_URL}images/who_logo.png`} style={{ height: 54, marginTop: 5 }} />
          </div>
          {friendDetail}
        </div>
      </div>
    );
  }
}

const mapStateToProps = state => ({
  setCurrentFriend: state.setCurrentFriendReducer,
});

export default connect(mapStateToProps)(ContactDetail);
